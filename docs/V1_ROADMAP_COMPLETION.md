...
| 0 | RLS on all core tables | Done | `0003_row_level_security.sql`, `security_assumptions_test.dart` |
...
| 3 | Notification preferences | Done | `0012`, `notification_preferences_screen.dart` |
...
| 6 | Version 1.0.0 | Done | `pubspec.yaml` |

## Known limitations

- Android `android/` folder generated in CI when not committed.
- FCM push delivery requires Firebase + `google-services.json`.
- `sync_conflicts` table is not auto-populated from the sync engine yet (UI/RPCs ready).
