...
abstract class ReportsRepository {
  Future<DashboardStats> fetchDashboardStats({required DateTime from, required DateTime to});
  Future<List<StaffSalesRow>> fetchStaffSalesReport({required DateTime from, required DateTime to});
  Future<List<ProductSalesRow>> fetchProductSalesReport({required DateTime from, required DateTime to});
}
...
class SupabaseReportsRepository implements ReportsRepository {
  ...
  Future<DashboardStats> fetchDashboardStats({required DateTime from, required DateTime to}) async {
    try {
      ...
      throw ReportsException(e.message.isNotEmpty ? e.message : 'Could not load dashboard.');
    } catch (_) {
  ...
  Future<List<StaffSalesRow>> fetchStaffSalesReport({required DateTime from, required DateTime to}) async {
    try {
      ...
      return (result as List)
          .map((row) => StaffSalesRow.fromJson(Map<String, dynamic>.from(row as Map)))
          .toList();
    } on PostgrestException catch (e) {
      throw ReportsException(e.message.isNotEmpty ? e.message : 'Could not load staff report.');
    } catch (_) {
  ...
  Future<List<ProductSalesRow>> fetchProductSalesReport({required DateTime from, required DateTime to}) async {
    try {
      ...
      return (result as List)
          .map((row) => ProductSalesRow.fromJson(Map<String, dynamic>.from(row as Map)))
          .toList();
    } on PostgrestException catch (e) {
      throw ReportsException(e.message.isNotEmpty ? e.message : 'Could not load product report.');
    } catch (_) {
